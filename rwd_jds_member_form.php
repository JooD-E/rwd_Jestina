...
<head>
    ...
    <title>J&middot;estina member</title>
    ...
    <script>
        function check_id()
        {
            window.open("check_id.php?id=" + document.member_form.id.value,
                "IDcheck", "left=200, top=200, height=60, scrollbars=no, resizable=yes"
            );
        }
        function check_input()
        {
            ...
            if(!document.member_form.userPw.value)
            {
                ...
                alert("비밀번호는 6~15자의 영문, 숫자, 특수기호를 혼합하여 사용하세요");
                return false;
            }

            if(!document.member_form.userPwConfirm.value)
            {
                alert("비밀번호 확인을 입력하세요")
                document.member_form.userPwConfirm.focus();
                return
            }

            if(document.member_form.userPw.value != document.member_form.userPwConfirm.value)
            {
                alert("비밀번호가 일치하지 않습니다. \n 다시 입력해주세요")
                document.member_form.userPwConfirm.focus();
                document.member_form.userPwconfirm.select();
                return
            }
            ...
            if(!document.member_form.userHp.value)
            {
                alert("휴대폰 번호를 입력하세요");
                document.member_form.userHp.focus();
                return;
            }
            if(!document.member_form.userAddr.value)
            {
                alert("주소를 입력하세요")
            }
        ...
        <div class="form-wrap">
            <form name="member_form" method="post" action="insert.php">
                ...
                <div class="submit-btn-area">
                    ...
                    <button type="submit" onclick="check_input();" class="btn-submit">가입하기</button>
                </div>
            </form>
        </div>
    </section>
